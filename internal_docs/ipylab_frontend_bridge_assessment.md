...
After this assessment, the user approved implementing the narrow request/reply interface, the tools it enables, and example notebooks. Version 0.1.6 follows that design without an ipylab dependency; see [the implementation contract](bundled_tools.md). Automatic context-selection controls remain deferred. The [dialoghelper capability catalog](dialoghelper_tool_catalog.md) and [cell/kernel context notes](cell_kernel_model_and_context_selection.md) retain the broader options and rationale.
