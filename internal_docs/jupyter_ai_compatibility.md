...
### Actual 0.1.12 update and browser shutdown follow-up

The actual Extension Manager updated the matching Python 3.14/Jupyter AI environment from 0.1.10 to 0.1.12 in **3.803 seconds**, returning HTTP 201 and both frontend/server restart requirements. All installed-tool checks then passed with the removed native search/AST dependencies absent. PyPI catalogue discovery still displayed its loading animation for 45 seconds while server health was HTTP 200; this is distinct from package installation.

Two browser/notebook-open SIGTERM shutdowns remained alive beyond ten seconds after logging extension and kernel shutdown. On the repeated case, a `faulthandler` SIGUSR2 dump showed exactly one surviving AnyIO worker idle at `queue.get()` in AnyIO **4.15.1**, and the main thread in CPython `threading._shutdown`. The post-shutdown dump did **not** show pip, XML-RPC, or ordinary executor workers. This disproves blaming a still-running Rust build for that specific observed shutdown wait. It does not identify which library created the AnyIO thread. AnyIO's worker is non-daemon and normally stops via a root-task completion callback; losing that cleanup before event-loop shutdown is a plausible mechanism, not a proven originating component.

A catalogue-only PTY probe, without a notebook/browser kernel, accepted two Ctrl-C signals and exited **0.818 seconds** after the second. A separate browser launch of the upgraded environment with `--LabApp.extension_manager=readonly` showed 0.1.12, AI Prompt and Configure AI, and exited within five seconds after the browser closed. The latter differs in both manager mode and browser-close timing; do not claim it isolates the PyPI manager as the cause. The subsequent matched 45-second readonly browser test reproduced the same thread wait with catalogue loading absent, so this mode is not a shutdown fix. The stack also does not establish Jupyter AI/MCP as the creator: Jupyter Server 2.21.1 itself calls `anyio.to_thread.run_sync` to list directories and parse notebooks in its ContentsManager. No personal environment or configuration was changed, no third-party installation source was patched, and only owned servers on 8897 were stopped.

Ignored probe records: `dist/update-0112-py314-browser-result.log`, `dist/catalogue-shutdown-0112/browser-server.log`, `dist/catalogue-shutdown-0112/pypi-result.json`, and `dist/catalogue-shutdown-0112/readonly-browser-result.log`. They contain temporary public/example data, not the user's notebooks or tokens. The matched readonly log is `dist/catalogue-shutdown-0112/readonly-matched-server.log`. The [official extension-manager documentation](https://jupyterlab.readthedocs.io/en/stable/user/extensions.html#extension-manager-implementations) documents the readonly mode.


A clean **core-only** Python 3.14 control (JupyterLab 4.6.4, Jupyter Server 2.21.1, AnyIO 4.15.1; no nbinlineai, Jupyter AI, FastMCP or MCP server packages installed) opened a notebook, waited for its kernel to become idle, remained open for 45 seconds, and exited normally after SIGTERM. Its logs are `dist/catalogue-shutdown-0112/core-result.log` and `core-server.log`. This narrows the reproduction to the fuller installed setup; it does not by itself identify one extension or dependency as responsible.


The final control disabled **only `jupyter_server_mcp`** at launch in the fuller environment, retained Jupyter AI and nbinlineai, used the readonly extension manager, and kept the browser/notebook open for the same 45 seconds. Startup logs confirmed no MCP extension/listener. The server still remained alive more than 10.5 seconds after SIGTERM, with the same idle AnyIO worker/main-thread shutdown stack. Thus neither the MCP server nor PyPI discovery is necessary for this reproduction. Another extension/dependency interaction remains unidentified; this is not evidence against Jupyter AI as a whole or nbinlineai specifically. Logs: `dist/catalogue-shutdown-0112/mcp-off-matched-result.log` and `mcp-off-matched-server.log`. The owned server was stopped; no further variants were run.
